...
import {AddAttendanForm, AttendantForm} from './AddAttendanFormComponent';
...
export class SimpleAttendantList extends React.Component<{}, {}> {
    ...
    render() {
        let attendants = this.state.attendants.map((x) => (<li key={x.name + x.surname}>{x.name} {x.surname}</li>));
        return(
            <div >
                <h2>List of Attendants</h2>
                <AddAttendanForm  onFormSubmit={this.onAttendantFormSubmit} />
                <ul>
                    {attendants}
                </ul>
            </div>
